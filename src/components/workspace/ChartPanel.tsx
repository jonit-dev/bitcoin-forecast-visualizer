...
export function ChartPanel({ title, subtitle, range, onRange, isPlaying, busy, onPlayToggle, settingsTriggerRef, settingsOpen, onOpenSettings, tall = false, children }: { title: string; subtitle: string; range: string; onRange(value: string): void; isPlaying: boolean; busy: boolean; onPlayToggle(): void; settingsTriggerRef: RefObject<HTMLButtonElement | null>; settingsOpen: boolean; onOpenSettings(): void; tall?: boolean; children: ReactNode }) {
  return <section className={tall ? 'chart-panel chart-panel-tall' : 'chart-panel'} aria-label="Forecast chart">
