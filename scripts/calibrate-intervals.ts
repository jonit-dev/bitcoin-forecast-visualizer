...
function main(): void {
  ...
  const report = {
    metadata: {
      ...
      dataset: { firstDate: ohlcv[0]?.date ?? '', lastDate: ohlcv.at(-1)?.date ?? '', rowCount: ohlcv.length },
    ...
    rows,
    note: 'Validation coverage is scored on a disjoint window. DIVERGENT rows never contribute a suggested shipped multiplier. CRPS and Winkler are absolute price-scale scores; PIT is not used for multiplier fitting.',
  };
...
function renderMarkdown(report: {
  metadata: { generatedAt: string; calibration: typeof INTERVAL_CALIBRATION_CONFIG; dataset: { firstDate: string; lastDate: string; rowCount: number } };
  status: string;
