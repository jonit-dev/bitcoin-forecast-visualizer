...
import { MIN_INTERVAL_ELIGIBLE_ROWS, PIT_HORIZONS, PIT_SEED, runPointInTimeBenchmark, type PitCandidateId } from '../src/lib/pointInTimeForecast';
...
const report = {
  metadata: { generatedAt, command: `npm run backtest:pit-core -- ${args.join(' ')}`.trim(), gitCommit: currentGitCommit, workingTreeDirty: workingTreeDirty(), sourceTreeDirty: sourceTreeDirty(), seed: PIT_SEED,
    ...
    dataset: { firstDate: dataset[0].date, lastDate: dataset.at(-1)!.date, rowCount: dataset.length, sha256: datasetSha256 },
    originStart, spacingDays, horizons: PIT_HORIZONS, candidate: candidate ?? null, pinballScale: 'absolute', deterministicContentSha256, deterministicRerunContract:'same data/config/seed produces the same content hash; generatedAt and artifact filename excluded' },
  methodology: { status: 'report-only', structuralFit: 'expanding point-in-time power-law fit through origin close', intervalCalibration: 'prior reconstructed-policy errors filtered by the shared purgeAndEmbargoResidualRows predicate; targetDate < originDate, horizon embargo, and minimum 30 eligible rows', benchmarkSchedule: 'identical origin/horizon rows for all available models', nestedSelection: 'six inner walk-forward folds wholly before each outer origin; candidates selected on inner target MALE only' },
  ...
  embargoAudit: { minimumEligibleRows: MIN_INTERVAL_ELIGIBLE_ROWS, totalExcludedByEmbargo: result.origins.reduce((sum, origin) => sum + origin.supervisedPolicy.excludedByEmbargo, 0), byHorizon: Object.fromEntries(PIT_HORIZONS.map(horizon => [horizon, result.origins.filter(origin => origin.horizonDays === horizon).reduce((sum, origin) => sum + origin.supervisedPolicy.excludedByEmbargo, 0)])) },
...
writeFileSync(`${base}.md`, `${render(report)}\n## Provenance state\n\n- Working tree dirty at generation start: ${report.metadata.workingTreeDirty ? 'yes' : 'no'}\n- Source tree dirty at generation start: ${report.metadata.sourceTreeDirty ? 'yes' : 'no'}\n\n## Embargo audit\n\n- Minimum eligible interval rows: ${report.embargoAudit.minimumEligibleRows}\n- Total rows withheld by the shared embargo predicate: ${report.embargoAudit.totalExcludedByEmbargo}\n- Excluded by horizon: ${Object.entries(report.embargoAudit.byHorizon).map(([horizon, count]) => `${horizon}d=${count}`).join(', ')}\n`);
...
function gitStatusEntries(): string[] { try { const output=execSync('git status --porcelain --untracked-files=all',{encoding:'utf8'}).trim(); return output?output.split('\n'):[]; } catch { return ['git status unavailable']; } }
function workingTreeDirty(): boolean { return gitStatusEntries().length>0; }
function sourceTreeDirty(): boolean { return gitStatusEntries().some(entry=>!entry.slice(3).startsWith('docs/reports/results/')); }
...
function aggregatePath(origins:typeof result.origins){
  const rows=origins.map(o=>o.candidate?.pathDiagnostics).filter(Boolean) as NonNullable<NonNullable<(typeof origins)[number]['candidate']>['pathDiagnostics']>[];
  const keys=['variance','lag1Autocorrelation','absoluteLag1Autocorrelation','signChangeRate','q05','q95','maximumDrawdown','maximumDrawdownDuration'] as const;
  const methods=['current-recent-window','moving-block','volatility-regime','state-space'];
  const diagnostics=Object.fromEntries(keys.map(k=>[k,{source:mean(rows.map(r=>r.source[k])),generated:mean(rows.map(r=>r.generated[k])),relativeDifference:mean(rows.map(r=>(r.generated[k]-r.source[k])/(Math.abs(r.source[k])||1))),bootstrapTolerance95:blockBootstrap(rows.map(r=>Math.abs(r.generated[k]-r.source[k])),1,PIT_SEED)}]));
  const pathValidityPass=rows.length>0&&Object.values(diagnostics).every(metric=>Number.isFinite(metric.relativeDifference)&&Math.abs(metric.relativeDifference)<=0.5);
  return{samples:rows.length,pathValidityPass,selectedMethod:'volatility-regime contiguous blocks',realizedVolatilityDistribution:{source:quantiles(rows.map(r=>Math.sqrt(r.source.variance))),generated:quantiles(rows.map(r=>Math.sqrt(r.generated.variance)))},diagnostics,methodComparison:methods.map(method=>({method,diagnostics:Object.fromEntries(keys.map(k=>[k,mean(rows.map(r=>r.comparisons.find(c=>c.method===method)!.diagnostics[k]))]))})),twoSampleTolerance:'per-origin absolute diagnostic deltas with moving bootstrap; tails, drawdown, dependence and realized volatility included',terminalCalibration:'horizon-scaled simulated q10/q50/q90 with pinball/NLL/coverage/width in horizon checks'};
}
