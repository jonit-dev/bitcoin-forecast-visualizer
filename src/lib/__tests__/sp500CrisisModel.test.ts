...
import { getCurrentCrisisRisk } from '../sp500CrisisModel';
...
describe('S&P 500 crisis challenger snapshot', () => {
  it('should expose the imported current score when the snapshot is valid', () => {
    const assessment = getCurrentCrisisRisk('2026-08-14');
    const current = assessment.snapshot.currentScore;

    expect(assessment.snapshot.source.archive).toBe('sp500-crisis-model-v2.zip');
    expect(assessment.snapshot.source.snapshotDate).toBe('2026-08-15');
  ...
  it('should preserve the OOS history, holdout uncertainty, and context-only verdict', () => {
    const snapshot = getCurrentCrisisRisk('2026-08-14').snapshot;
    expect(snapshot.oosHistory).toHaveLength(1025);
    expect(snapshot.oosHistory[0]).toMatchObject({ date: '2000-01-07', target: 0 });
    expect(snapshot.oosHistory.at(-1)).toMatchObject({ date: '2025-12-26', target: 0 });
    expect(snapshot.holdout.period).toBe('2016-2025');
    expect(snapshot.holdout.uncertainty.brier_improvement.ci_95[0]).toBeLessThan(0);
    expect(snapshot.holdout.uncertainty.brier_improvement.ci_95[1]).toBeGreaterThan(0);
    expect(snapshot.runtime.mode).toBe('shadow');
    expect(snapshot.runtime.verdict).toBe('context-only');
    expect(snapshot.runtime.promotionStatus).toBe('not-promoted');
  });
});
