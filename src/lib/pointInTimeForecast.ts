...
function contiguousSums(values:number[],h:number){const sums:number[]=[];for(let i=0;i+h<=values.length;i++)sums.push(values.slice(i,i+h).reduce((a,b)=>a+b,0));return sums;}
