...
export interface OverlayControl { id: string; label: string; group: 'Price' | 'Forecast' | 'Indicators' | 'Bitcoin context'; checked: boolean; onChange(): void; description: string }
export function ChartSettings({ open, controls, onClose, triggerRef }: { open: boolean; controls: OverlayControl[]; onClose(): void; triggerRef: RefObject<HTMLButtonElement | null> }) {
  ...
  return <div className="chart-settings-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section className="chart-settings" role="dialog" aria-modal="true" aria-labelledby={titleId}>
    ...
    {(['Price', 'Forecast', 'Indicators', 'Bitcoin context'] as const).map((group) => controls.some((control) => control.group === group) && <fieldset key={group}><legend>{group}</legend>{controls.filter((control) => control.group === group).map((control) => <label key={control.id} title={control.description}><span><strong>{control.label}</strong><small>{control.description}</small></span><input type="checkbox" role="switch" checked={control.checked} onChange={control.onChange} /></label>)}</fieldset>)}
  </section></div>;
}
